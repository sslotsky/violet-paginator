...
function register(config) {
  var simple = (0, _simpleComposables2.default)(config.listId);
  var updateAsync = function updateAsync(id, data, update) {
    return function (dispatch, getState) {
      var item = (0, _stateManagement2.default)(getState(), config.listId).get('results').find(function (r) {
        return r.get(identifier) === id;
      });

      dispatch(simple.updateItem(id, data));
      dispatch(simple.updatingItem(id));
      return update().then(function (serverUpdate) {
        return dispatch(simple.updateItem(id, serverUpdate));
      }).catch(function (err) {
