...
function setPageSize(state, action) {
  return (0, _reduxResolver.updateListItem)(state, action.id, function (p) {
    return p.set('pageSize', action.size);
  });
}

function fetching(state, action) {
  return (0, _reduxResolver.updateListItem)(state, action.id, function (p) {
    return p.set('isLoading', true);
  });
}
...
function updatingItem(state, action) {
  return (0, _reduxResolver.updateListItem)(state, action.id, function (p) {
    return p.setIn(['updating', 'id'], action.itemId);
  });
}
...
exports.default = (0, _reduxResolver.resolveEach)(initialState, (_resolveEach = {}, _defineProperty(_resolveEach, actionTypes.INITIALIZE_PAGINATOR, initialize), _defineProperty(_resolveEach, actionTypes.PREVIOUS_PAGE, prev), _defineProperty(_resolveEach, actionTypes.NEXT_PAGE, next), _defineProperty(_resolveEach, actionTypes.GO_TO_PAGE, goToPage), _defineProperty(_resolveEach, actionTypes.SET_PAGE_SIZE, setPageSize), _defineProperty(_resolveEach, actionTypes.FETCH_RECORDS, fetching), _defineProperty(_resolveEach, actionTypes.RESULTS_UPDATED, updateResults), _defineProperty(_resolveEach, actionTypes.RESULTS_UPDATED_ERROR, error), _defineProperty(_resolveEach, actionTypes.TOGGLE_FILTER_ITEM, toggleFilterItem), _defineProperty(_resolveEach, actionTypes.SET_FILTER, setFilter), _defineProperty(_resolveEach, actionTypes.SORT_CHANGED, sortChanged), _defineProperty(_resolveEach, actionTypes.UPDATING_ITEM, updatingItem), _defineProperty(_resolveEach, actionTypes.UPDATE_ITEM, updateItem), _defineProperty(_resolveEach, actionTypes.REMOVE_ITEM, removeItem), _resolveEach));